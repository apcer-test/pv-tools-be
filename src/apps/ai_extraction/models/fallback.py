from sqlalchemy import String, ForeignKey, UniqueConstraint, DECIMAL, JSON
...
class FallbackStepModel(Base, ULIDPrimaryKeyMixin, TimeStampMixin):
    ...
    chain_id: Mapped[str] = mapped_column(ForeignKey("fallback_chain.id", ondelete="CASCADE"))
    seq_no: Mapped[int] = mapped_column()
    model_id: Mapped[str] = mapped_column(ForeignKey("llm_model.id"))
    llm_credential_id: Mapped[str | None] = mapped_column(ForeignKey("llm_credential.id"))
    ...
    extraction_audits: Mapped[List["ExtractionAuditModel"]] = relationship(
        ...
        back_populates="chain_step"
    )

    # Unique constraint for chain_id and seq_no combination
    __table_args__ = (
        UniqueConstraint('chain_id', 'seq_no', name='uq_chain_seq_no'),
    )
