...
class PromptTemplateCreateRequest(CamelCaseModel):
    ...
    temperature: Union[float, str] = Field(0.0, description="Sampling temperature (0.0 to 1.0)")
    top_p: Union[float, str] = Field(1.0, description="Nucleus sampling parameter (0.0 to 1.0)")
...
class ExtractionRequest(CamelCaseModel):
    ...
    request_id: ULID = Field(..., description="External correlation ULID for idempotency")
    file_path: str = Field(..., description="Path to input document (PDF/TXT)")
    doc_type: DocType = Field(..., description="Document type driving prompt & schema selection")
...
class LLMModelCreateRequest(CamelCaseModel):
    ...
    input_price_1k: Union[Decimal, str] = Field(..., description="Input price per 1K tokens")
    output_price_1k: Union[Decimal, str] = Field(..., description="Output price per 1K tokens")
...
class LLMCredentialCreateRequest(CamelCaseModel):
    ...
    provider_id: ULID = Field(..., description="Provider ID")
    alias: Optional[str] = Field(None, description="Credential alias for identification")
