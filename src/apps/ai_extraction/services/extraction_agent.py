...
from apps.ai_extraction.models.llm import ExtractionAgentModel, LLMCredentialModel, LLMModel
...
class ExtractionAgentService:
    ...
    async def validate_references(self, doc_type_id: ULID, template_id: ULID, chain_id: ULID, credential_id: ULID) -> None:
    ...
    async def create_agent(self, request: ExtractionAgentCreateRequest) -> SuccessResponse:
        ...
        await self.validate_references(str(request.doc_type_id), str(request.template_id), str(request.chain_id), str(request.credential_id))
    ...
    async def update_agent(self, agent_id: ULID, doc_type_id: ULID, template_id: ULID, chain_id: ULID, credential_id: ULID, code: str, description: str, is_active: bool, seq_no: int) -> SuccessResponse:
        ...
        # Validate that all referenced IDs exist
        await self.validate_references(doc_type_id, template_id, chain_id, credential_id)
