...
from apps.ai_extraction.exceptions import ProviderError, ValidationError, ChainExhaustedError
...
class FallbackManager:
    ...
    async def execute_with_agent(self,
        ...
        if not agent.fallback_chain:
            ...
            self.logger.error(f"Fallback chain error - RequestID: {request_id}, Agent: {agent.code}, Error: {error_msg}")
        ...
        if agent.preferred_model:
            ...
            if preferred_step_index is not None:
                ...
                self.logger.info(f"Starting from preferred model step {start_index + 1} - RequestID: {request_id}, Agent: {agent.code}, Preferred Model: {agent.preferred_model}")
            else:
                self.logger.warning(f"Preferred model '{agent.preferred_model}' not found in fallback chain - RequestID: {request_id}, Agent: {agent.code}, Starting from first step")
        
        self.logger.info(f"Executing fallback chain - RequestID: {request_id}, Agent: {agent.code}, Steps: {len(steps)}, Start Index: {start_index}")
        ...
        for step_index, step in enumerate(steps[start_index:], start=start_index):
            self.logger.info(f"Executing step {step_index + 1}/{len(steps)} - RequestID: {request_id}, Agent: {agent.code}, Model: {step.model.name}")
            
            # Get the credential from the step
            credential = await self._get_credential(step.llm_credential_id)
            if not credential:
                self.logger.warning(f"Credential not found for step {step_index + 1} - RequestID: {request_id}, Agent: {agent.code}, CredentialID: {step.llm_credential_id}")
                continue

            if step.model.is_deprecated:
                self.logger.warning(f"Skipping deprecated model - RequestID: {request_id}, Agent: {agent.code}, Model: {step.model.name}")
                continue
            ...
            try:
                ...
                self.logger.info(f"LLM call successful - RequestID: {request_id}, Agent: {agent.code}, Model: {step.model.name}, Tokens: {llm_result.tokens_prompt}/{llm_result.tokens_completion}")
                ...
                field_count = len(validated_data) if isinstance(validated_data, dict) else 0
                ...
                self.logger.info(f"Validation successful - RequestID: {request_id}, Agent: {agent.code}, Fields: {field_count}")
                ...
                self.logger.warning(f"Step {step_index + 1} failed - RequestID: {request_id}, Agent: {agent.code}, Model: {step.model.name}, Error: {str(e)}")
                ...
                if agent.preferred_model and step.model.name == agent.preferred_model:
                    self.logger.warning(f"Preferred model '{agent.preferred_model}' failed validation - RequestID: {request_id}, Agent: {agent.code}")
                ...
                self.logger.error(f"Unexpected error in step {step_index + 1} - RequestID: {request_id}, Agent: {agent.code}, Model: {step.model.name}, Error: {str(e)}", exc_info=True)
                continue
        
        # All steps failed
        error_msg = f"Chain exhausted for agent {agent.code}. Last error: {str(last_error)}" if last_error else f"Chain exhausted for agent {agent.code}"
        self.logger.error(f"All steps failed - RequestID: {request_id}, Agent: {agent.code}, Error: {error_msg}")
        raise ChainExhaustedError(error_msg)
    ...
    async def _load_agent_for_doc_type(self, doc_type: DocType) -> ExtractionAgentModel | None:
    ...
    async def _get_credential(self, credential_id: ULID) -> LLMCredentialModel | None:
        ...
        credential = await self.session.scalar(
            ...
            .options(
                ...
                selectinload(LLMCredentialModel.fallback_steps)
                .selectinload(FallbackStepModel.model)
            )
        )
    ...
    async def _call_llm_with_step(self,
    ...
    async def _log_step_attempt(self,
        step: FallbackStepModel,
        request_id: ULID,
        error: str | None = None,
