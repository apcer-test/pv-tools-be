...
from apps.ai_extraction.models.llm import LLMCredentialModel, LLMModel, LLMProviderModel
...
class LLMService:
    ...
    async def create_provider(
        ...
        provider = LLMProviderModel(
            name=request.name, base_url=request.base_url, is_active=request.is_active
        )
    ...
    async def create_model(self, request: LLMModelCreateRequest) -> LLMModelResponse:
        ...
        provider = await self.session.scalar(
            select(LLMProviderModel).where(
                LLMProviderModel.id == str(request.provider_id)
            )
        )
    ...
    async def create_credential(
        self, request: LLMCredentialCreateRequest
    ) -> LLMCredentialResponse:
        """
    ...
    async def get_all_credentials(
        ...
        """
        stmt = select(LLMCredentialModel).options(
