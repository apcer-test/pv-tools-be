...
class LLMGateway:
    ...
    async def call(
        *,
        ...
        # Log call start
        LLMGateway._logger.info(
            f"LLM call started - Provider: {provider}, Model: {model}, MaxTokens: {max_tokens}, Temperature: {temperature}"
        )
        ...
        try:
            ...
            if isinstance(prompt, list):
                ...
                LLMGateway._logger.debug(
                    f"Using content parts prompt - Provider: {provider}, Model: {model}, Parts: {len(prompt)}"
                )
            else:
                # If prompt is a string, wrap it in a message
                call_params["messages"] = [{"role": "user", "content": prompt}]
                LLMGateway._logger.debug(
                    f"Using text prompt - Provider: {provider}, Model: {model}, Length: {len(prompt)}"
                )
            ...
            # Log successful call
            LLMGateway._logger.info(
                f"LLM call successful - Provider: {provider}, Model: {model}, Tokens: {usage.prompt_tokens}/{usage.completion_tokens}, Cost: ${cost_usd}, Latency: {latency_ms}ms"
            )
            ...
            LLMGateway._logger.error(
                f"Authentication error - Provider: {provider}, Model: {model}, Error: {error_msg}"
            )
            raise ProviderError("AUTH_FAIL", error_msg)
        except Timeout as e:
            error_msg = f"Request timeout: {str(e)}"
            LLMGateway._logger.error(
                f"Timeout error - Provider: {provider}, Model: {model}, Error: {error_msg}"
            )
            raise ProviderError("TIMEOUT", error_msg)
        except RateLimitError as e:
            error_msg = f"Rate limit exceeded: {str(e)}"
            LLMGateway._logger.error(
                f"Rate limit error - Provider: {provider}, Model: {model}, Error: {error_msg}"
            )
            raise ProviderError("RATE_LIMIT", error_msg)
        except Exception as e:
            error_msg = f"LLM API call failed: {str(e)}"
            LLMGateway._logger.error(
                f"API call error - Provider: {provider}, Model: {model}, Error: {error_msg}",
                exc_info=True,
            )
