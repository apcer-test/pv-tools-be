...
from typing import Union, Annotated, Dict, List, Any, Optional
...
from apps.ai_extraction.schemas.response import ExtractionResult, ExtractionError, ExtractionDataResult
...
from apps.ai_extraction.exceptions import PreProcessError, ExtractionFailedError
from apps.ai_extraction.models.llm import LLMCredentialModel, ExtractionAgentModel
...
class Orchestrator:
    ...
    async def extract_document(self,
        ...
        try:
            self.logger.info(f"Starting document extraction - RequestID: {request_id}, DocType: {doc_type}, File: {file_path}")
            
            # 1. Pre-processing: Convert document to text
            self.logger.info(f"Loading document for preprocessing - RequestID: {request_id}")
            self.preprocessor.load(file_path)
            
            # 2. Get all active extraction agents for this document type
            self.logger.info(f"Loading extraction agents - RequestID: {request_id}, DocType: {doc_type}")
            ...
            if not doc_type_obj or not doc_type_obj.extraction_agents:
                ...
                self.logger.error(f"Agent configuration error - RequestID: {request_id}, Error: {error_msg}")
            ...
            if not active_agents:
                error_msg = f"No active extraction agents found for doc_type: {doc_type}"
                self.logger.error(f"No active agents - RequestID: {request_id}, Error: {error_msg}")
                raise ExtractionFailedError(error_msg)

            self.logger.info(f"Found {len(active_agents)} active agents - RequestID: {request_id}")
            ...
            self.logger.info(f"Preparing file uploads for LLM providers - RequestID: {request_id}")
            
            for step in active_agents[0].fallback_chain.fallback_steps:
                credential = await self.fallback_manager._get_credential(step.llm_credential_id)
                if not credential:
                    continue
                ...
                if credential.id not in credential_files:
                    try:
                        ...
                        self.logger.info(f"File uploaded for credential {credential.id} - RequestID: {request_id}, FileID: {uploaded_file.id}")
                    except Exception as e:
                        self.logger.warning(f"Failed to upload file for credential {credential.id} - RequestID: {request_id}, Error: {str(e)}")
                        continue

            # Now process each agent with their respective uploaded files
            for agent in active_agents:
                try:
                    self.logger.info(f"Processing agent {agent.code} - RequestID: {request_id}")
                    ...
                    prompt = await self.prompt_registry.render_by_template_id(template_id)
                    self.logger.info(f"Template rendered for agent {agent.code} - RequestID: {request_id}")
                    ...
                    if not uploaded_file_id:
                        error_msg = f"No valid uploaded file found for agent {agent.code}"
                        self.logger.error(f"File upload error - RequestID: {request_id}, Agent: {agent.code}, Error: {error_msg}")
                        raise ExtractionFailedError(error_msg)
                    
                    # Create content parts with file reference
                    content_parts = [
                        {"type": "text", "text": prompt},
                        {"type": "file", "file": {"file_id": uploaded_file_id, "format": MIME_PDF}}
                    ]
                    ...
                    if isinstance(agent_result, ExtractionDataResult):
                        ...
                        await logger_instance.log_agent_complete(
                            ...
                            agent_id=agent.id,
                            doc_type=doc_type,
                            field_count=len(agent_result.data) if agent_result.data else 0,
                    ...
                    self.logger.error(f"Agent execution failed - RequestID: {request_id}, Agent: {agent.code}, Error: {error_msg}")
            ...
            if not extraction_results:
                ...
                self.logger.error(f"All agents failed - RequestID: {request_id}, Error: {error_msg}")
                raise ExtractionFailedError(error_msg)
            ...
            self.logger.info(f"Extraction completed successfully - RequestID: {request_id}, Agents: {successful_agents}/{len(active_agents)}, Fields: {total_fields}, Cost: ${total_cost}, Latency: {total_latency}ms")
            ...
            self.logger.error(f"Preprocessing failed - RequestID: {request_id}, Error: {str(e)}")
            ...
            self.logger.error(f"Extraction failed - RequestID: {request_id}, Error: {str(e)}")
            return await logger_instance.log_error(
                request_id=request_id,
            ...
            self.logger.error(f"Unexpected error - RequestID: {request_id}, Error: {str(e)}", exc_info=True)
            return await logger_instance.log_error(
                request_id=request_id,
