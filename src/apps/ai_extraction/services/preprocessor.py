...
class PreProcessor:
    ...
    def redact_pdf(src: Path, dst: Path, patterns: List[str]) -> None:
        ...
        """
        PreProcessor._logger.info(f"Starting PDF redaction - Source: {src}, Destination: {dst}, Patterns: {len(patterns)}")
        ...
        try:
            ...
            for page_num, page in enumerate(doc):
                ...
                if page_redactions > 0:
                    ...
                    PreProcessor._logger.debug(f"Applied {page_redactions} redactions on page {page_num + 1}")
            
            doc.save(dst)
            doc.close()
            
            PreProcessor._logger.info(f"PDF redaction completed - Total redactions: {redaction_count}")
            
        except Exception as e:
            error_msg = f"PDF redaction failed: {str(e)}"
            PreProcessor._logger.error(f"PDF redaction error - Source: {src}, Error: {error_msg}", exc_info=True)
    ...
    def load(path: Path) -> str:
        ...
        try:
            ...
            if file_extension == ".pdf":
                ...
                PreProcessor._logger.error(f"Unsupported file type - Path: {path}, Extension: {file_extension}")
                raise PreProcessError(error_msg)
                
        except Exception as e:
            error_msg = f"Text extraction failed: {str(e)}"
            PreProcessor._logger.error(f"Text extraction error - Path: {path}, Error: {error_msg}", exc_info=True)
            raise PreProcessError(error_msg)
    ...
    def _extract_pdf_text(path: Path) -> str:
        ...
        try:
            ...
            for page_num, page in enumerate(doc):
                ...
                PreProcessor._logger.debug(f"Page {page_num + 1} processed - Characters: {len(page_text)}")
            ...
            PreProcessor._logger.info(f"PDF text extraction completed - Path: {path}, Pages: {page_count}, Characters: {total_chars}, Words: {word_count}")
                
            return text.strip()
            
        except Exception as e:
            error_msg = f"PDF text extraction failed: {str(e)}"
            PreProcessor._logger.error(f"PDF text extraction error - Path: {path}, Error: {error_msg}", exc_info=True)
            raise PreProcessError(error_msg)
    ...
    def _extract_txt_content(path: Path) -> str:
        ...
        try:
            ...
            PreProcessor._logger.info(f"Text file extraction completed - Path: {path}, Characters: {len(content)}, Words: {word_count}")
                
            return content.strip()
            
        except UnicodeDecodeError:
            PreProcessor._logger.warning(f"UTF-8 decoding failed, trying latin-1 - Path: {path}")
            # Try with different encoding
            try:
                content = path.read_text(encoding="latin-1")
                word_count = len(content.split())
                PreProcessor._logger.info(f"Text file extraction completed with latin-1 encoding - Path: {path}, Characters: {len(content)}, Words: {word_count}")
                return content.strip()
            except Exception as e:
                error_msg = f"Text file encoding error: {str(e)}"
                PreProcessor._logger.error(f"Text file encoding error - Path: {path}, Error: {error_msg}")
                raise PreProcessError(error_msg)
        except Exception as e:
            error_msg = f"Text file read error: {str(e)}"
            PreProcessor._logger.error(f"Text file read error - Path: {path}, Error: {error_msg}")
            raise PreProcessError(error_msg)
        ...
        try:
            ...
            if file_type == "pdf":
                try:
                    ...
                    page_count = len(doc)
                    doc.close()
                    PreProcessor._logger.debug(f"PDF page count determined - Pages: {page_count}")
                except Exception as e:
                    PreProcessor._logger.warning(f"Failed to determine PDF page count - Path: {path}, Error: {str(e)}")
            
            PreProcessor._logger.info(f"File info completed - Path: {path}, Type: {file_type}, Pages: {page_count}, Words: {word_count}")
            ...
            PreProcessor._logger.error(f"File info error - Path: {path}, Error: {error_msg}", exc_info=True)
            raise PreProcessError(error_msg) 