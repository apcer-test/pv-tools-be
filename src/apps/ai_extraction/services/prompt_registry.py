...
class PromptRegistry:
    ...
    async def render(self, doc_type: DocType) -> Tuple[str, ULID]:
        ...
        try:
            ...
            if not template_obj:
                raise TemplateNotFoundError(f"No active template found for doc_type: {doc_type}")
    ...
    async def render_by_template_id(self, template_id: ULID) -> str:
        ...
        try:
            ...
            if not template_obj:
                raise TemplateNotFoundError(f"Template not found with ID: {template_id}")
    ...
    async def create_template(self, template_data: PromptTemplateCreateRequest) -> PromptTemplateResponse:
    ...
    def _to_response_model(self, template: PromptTemplateModel) -> PromptTemplateResponse:
    ...
    async def _get_active_template(self, doc_type: DocType) -> PromptTemplateModel | None:
