...
class SchemaValidator:
    ...
    @staticmethod
    def validate(json_str: str, doc_type: DocType, agent_code: Optional[str] = None) -> Dict[str, Any]:
        ...
        """
        SchemaValidator._logger.info(f"Starting JSON validation - DocType: {doc_type}, Agent: {agent_code}, Length: {len(json_str)}")
        ...
        try:
            ...
            if match:
                ...
                SchemaValidator._logger.debug(f"Extracted JSON from markdown code block - DocType: {doc_type}")
            
            data = json.loads(json_str)
            SchemaValidator._logger.info(f"JSON parsing successful - DocType: {doc_type}, Agent: {agent_code}")
            
        except json.JSONDecodeError as e:
            error_msg = f"Invalid JSON: {str(e)}"
            SchemaValidator._logger.error(f"JSON parsing failed - DocType: {doc_type}, Agent: {agent_code}, Error: {error_msg}")
        ...
        SchemaValidator._logger.info(f"Validation completed - DocType: {doc_type}, Agent: {agent_code}, Fields: {field_count}")
        return data
    ...
    def validate_with_result(json_str: str, doc_type: DocType) -> ValidationResult:
        ...
        """
        SchemaValidator._logger.info(f"Starting detailed validation - DocType: {doc_type}, Length: {len(json_str)}")
        ...
        try:
            ...
            try:
                data = json.loads(json_str)
                SchemaValidator._logger.debug(f"JSON parsing successful - DocType: {doc_type}")
            except json.JSONDecodeError:
                # Try to repair the JSON
                SchemaValidator._logger.warning(f"JSON parsing failed, attempting repair - DocType: {doc_type}")
            ...
            if schema_cls:
                try:
                    ...
                    SchemaValidator._logger.info(f"Schema validation successful - DocType: {doc_type}, Fields: {len(validated_data)}")
                except Exception as e:
                    error_msg = f"Schema validation failed: {str(e)}"
                    SchemaValidator._logger.error(f"Schema validation failed - DocType: {doc_type}, Error: {error_msg}")
                    raise ValidationError(error_msg)
            else:
                # No specific schema - basic validation
                if not isinstance(data, dict):
                    error_msg = "Expected JSON object, got other type"
                    SchemaValidator._logger.error(f"Basic validation failed - DocType: {doc_type}, Error: {error_msg}")
                    raise ValidationError(error_msg)
                validated_data = data
                SchemaValidator._logger.info(f"Basic validation successful - DocType: {doc_type}, Fields: {len(validated_data)}")
            ...
            SchemaValidator._logger.error(f"Validation failed - DocType: {doc_type}, Error: {str(e)}")
            return ValidationResult(
                is_valid=False,
                validated_data=None,
                errors=[str(e)],
            ...
            SchemaValidator._logger.error(f"Unexpected validation error - DocType: {doc_type}, Error: {error_msg}", exc_info=True)
            return ValidationResult(
                is_valid=False,
                validated_data=None,
                errors=[error_msg],
    ...
    def repair_json(json_str: str) -> str:
        ...
        """
        SchemaValidator._logger.info(f"Attempting JSON repair - Length: {len(json_str)}")
    ...
    def _basic_json_cleanup(json_str: str) -> str:
        ...
        Returns:
            Cleaned JSON string
        """
        SchemaValidator._logger.debug(f"Performing basic JSON cleanup - Length: {len(json_str)}")
        ...
        SchemaValidator._logger.debug(f"JSON cleanup completed - New length: {len(cleaned)}")
        return cleaned
    ...
    def get_schema_info(doc_type: DocType) -> Dict[str, Any]:
        ...
        if not schema_cls:
            SchemaValidator._logger.warning(f"No schema defined for doc type: {doc_type}")
            return {
                "exists": False,
                "doc_type": doc_type,
        ...
        try:
            ...
            SchemaValidator._logger.error(f"Schema introspection failed - DocType: {doc_type}, Error: {error_msg}")
            return {
                "exists": True,
                "doc_type": doc_type,
                "error": str(e),
