from sqlalchemy import String, Text, ForeignKey, JSON, Integer, Enum
...
class DocumentIntakeHistory(Base, ULIDPrimaryKeyMixin, TimeStampMixin):
    ...
    # Core fields
    status: Mapped[DocumentIntakeStatus] = mapped_column(Enum(DocumentIntakeStatus), default=DocumentIntakeStatus.PENDING)
    source: Mapped[DocumentIntakeSource] = mapped_column(Enum(DocumentIntakeSource), default=DocumentIntakeSource.USER_UPLOAD)
    ...
    # Processing metadata
    processing_started_at: Mapped[Optional[str]] = mapped_column(String(50))  # ISO timestamp
    processing_completed_at: Mapped[Optional[str]] = mapped_column(String(50))  # ISO timestamp
    ...
    doc_type: Mapped["DocTypeModel"] = relationship(
        "DocTypeModel", 
        back_populates="document_intakes"
    )
