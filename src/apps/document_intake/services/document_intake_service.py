...
from apps.ai_extraction.schemas.response import ExtractionResult, ExtractionError
...
from apps.document_intake.models.document_intake import DocumentIntakeHistory, DocumentIntakeStatus, DocumentIntakeSource
...
class DocumentIntakeService:
    ...
    async def update_intake_status(
        ...
        intake_record = await self.session.scalar(
            select(DocumentIntakeHistory).where(DocumentIntakeHistory.request_id == request_id)
        )
        
        if not intake_record:
            raise ValueError(f"Document intake record with request_id {request_id} not found")
    ...
    async def process_document(
        ...
        await self.update_intake_status(str(request_id), DocumentIntakeStatus.IN_PROGRESS)
        ...
        try:
            ...
            self.logger.info(f"Starting file validation - RequestID: {request_id}, File: {file.filename}")
            ...
            if file_extension not in supported_extensions:
                ...
                self.logger.error(f"File validation failed - RequestID: {request_id}, Error: {error_msg}")
            ...
            self.logger.info(f"Starting preprocessing - RequestID: {request_id}, File: {file.filename}")
            await audit_logger.log_preprocessing_start(request_id=request_id, file_path=str(file.filename), doc_type=doc_type)
            ...
            try:
                ...
                with tempfile.NamedTemporaryFile(suffix=file_extension, delete=False) as temp_file:
                    ...
                    # Log file creation
                    self.logger.info(f"Temporary file created - RequestID: {request_id}, Path: {temp_file_path}, Size: {len(content)} bytes")
                ...
                self.logger.error(f"File creation failed - RequestID: {request_id}, Error: {error_msg}")
            ...
            self.logger.info(f"Starting AI extraction - RequestID: {request_id}, DocType: {doc_type}")
            ...
            if isinstance(result, ExtractionResult):
                await self.update_intake_status(str(request_id), DocumentIntakeStatus.COMPLETED)
                self.logger.info(f"AI extraction successful - RequestID: {request_id}, Fields: {len(result.data) if result.data else 0}")
                ...
                self.logger.error(f"AI extraction failed - RequestID: {request_id}, Error: {result.error_message}")
                ...
                self.logger.error(f"Unexpected result type - RequestID: {request_id}, Type: {type(result)}")
            ...
            self.logger.error(f"Document processing failed - RequestID: {request_id}, Error: {error_msg}", exc_info=True)
            ...
            if temp_file_path and temp_file_path.exists():
                try:
                    ...
                    self.logger.info(f"Temporary file cleaned up - RequestID: {request_id}, Path: {temp_file_path}")
                except Exception as cleanup_error:
                    self.logger.warning(f"Failed to cleanup temporary file - RequestID: {request_id}, Path: {temp_file_path}, Error: {str(cleanup_error)}")
