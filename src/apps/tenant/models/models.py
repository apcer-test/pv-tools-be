...
from core.utils.mixins import ULIDPrimaryKeyMixin, TimeStampMixin, UserMixin
...
class TenantUsers(Base, TimeStampMixin, UserMixin):
    ...
    tenant: Mapped["Tenant"] = relationship("Tenant", back_populates="tenant_users", foreign_keys="[TenantUsers.tenant_id]")
    user: Mapped["UserModel"] = relationship("UserModel", back_populates="tenant_users", foreign_keys="[TenantUsers.user_id]")
    ...
    @classmethod
    def create(
        cls, tenant_id: str, user_id: str, role: str = RoleType.ADMIN
    ) -> Self:
